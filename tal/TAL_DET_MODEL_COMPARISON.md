...
**PoseC3D Balanced Key Finding:** Balanced sampling achieves non-zero rocking F1 (2.76%) — the only PoseC3D variant to detect rocking at all. High variance in spinning (0-58% across folds) suggests sensitivity to class distribution.
...
- **V-JEPA** best for rocking detection (5.4% F1 vs 0% for others)
- **PoseC3D Balanced** achieves non-zero rocking (2.76%) with pose-only input
...
- Balanced sampling in skeleton model + RGB fusion = optimal TAL
...
### PoseC3D Balanced Results
- `pyskl/work_dirs/posec3d/tal/cv_4class_5class_ce_balanced/cv_summary.json` — PoseC3D Balanced CV summary
