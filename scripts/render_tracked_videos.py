#!/usr/bin/env python3
"""
Render videos with tracked target overlays (bounding boxes + pose skeletons).

This script:
1. Reads a CSV containing video metadata (must have SourceFile column)
2. Accepts multiple parent directories to search for tracking/target data
3. Accepts a video base directory to resolve video paths from CSV SourceFile
4. For each video in the CSV:
   - Resolves video path by joining video_base_dir with SourceFile (with .mp4 suffix)
   - Loads target identification JSON to find the matched track
   - Locates the corresponding H5 track file
   - Renders the entire video with bbox and COCO pose skeleton overlays
5. Processes videos sequentially to avoid segfaults

Usage:
    python render_tracked_videos.py /path/to/RMM.csv \
        --dirs /path/to/pipeline_outputs/dir1 /path/to/dir2 \
        --video-dir /path/to/videos/base/directory \
...
def resolve_video_config(row: pd.Series, parent_dirs: List[Path], video_base_dir: Optional[Path]) -> Optional[VideoConfig]:
    ...
    if not target_data:
        logger.warning(f"[{video_id}] No target identification JSON found")
        return None
    ...
    if not matched_track:
        logger.warning(f"[{video_id}] No matched track found in target JSON")
        return None
    ...
    if not video_path:
        logger.warning(f"[{video_id}] Video file not found: {source_file}")
        return None
    ...
    if not tracking_dir:
        ...
        tracking_dir = find_dir_in_dirs(tracking_dirname, parent_dirs, "tracking")
        if not tracking_dir:
            logger.warning(f"[{video_id}] Tracking directory not found: {tracking_dirname}")
            return None

    # Find H5 track file
    h5_path = resolve_track_path(tracking_dir, track_id)
    if not h5_path:
        logger.warning(f"[{video_id}] Track H5 file not found for track_id={track_id}")
        return None
...
def render_video(config: VideoConfig, output_dir: Path) -> bool:
    """Render a single video with track overlays"""
    logger.info(f"[{config.video_id}] Starting render (track_id={config.track_id})")

    # Load track data
    try:
        track = load_track_from_h5(config.h5_path)
    except Exception as e:
        logger.error(f"[{config.video_id}] Failed to load track H5: {e}")
        return False
    ...
    if not frame_data:
        logger.warning(f"[{config.video_id}] No drawable track data, skipping")
        return False
    ...
    if not cap.isOpened():
        logger.error(f"[{config.video_id}] Cannot open video: {config.video_path}")
        return False
    ...
    if not width or not height or width <= 0 or height <= 0:
        logger.error(f"[{config.video_id}] Invalid video dimensions: {width}x{height}")
        cap.release()
        return False
    ...
    if not test_ret or test_frame is None:
        logger.error(f"[{config.video_id}] Cannot read frames from video")
        cap.release()
        return False
    ...
    output_path = output_dir / f"{config.video_id}_tracked.mp4"
    ...
    try:
        ...
        logger.error(f"[{config.video_id}] ffmpeg not found in PATH")
        cap.release()
        return False
    ...
    try:
        with tqdm(total=total_frames, desc=f"Rendering {config.video_id}") as pbar:
            while frame_count < max_frames:
                try:
                    ...
                    if consecutive_errors >= max_consecutive_errors:
                        logger.error(f"[{config.video_id}] Too many consecutive read errors")
                        break
                    continue
                ...
                if data:
                    try:
                        ...
                        logger.warning(f"[{config.video_id}] Error drawing frame {current_frame}: {e}")

                # Write frame to ffmpeg
                try:
                    proc.stdin.write(frame.tobytes())
                except BrokenPipeError:
                    logger.error(f"[{config.video_id}] FFmpeg terminated unexpectedly at frame {current_frame}")
                    break

                pbar.update(1)

    except Exception as e:
        logger.error(f"[{config.video_id}] Unexpected error in render loop: {e}")
        return False
        ...
        try:
            ...
            logger.warning(f"[{config.video_id}] Error releasing video capture: {e}")
        ...
        if proc:
            try:
                ...
                logger.warning(f"[{config.video_id}] Error closing ffmpeg: {e}")
                try:
                    proc.kill()
                except:
                    pass

    logger.info(f"[{config.video_id}] Rendered successfully: {output_path}")
    return True


def main():
    parser = argparse.ArgumentParser(
        description="Render videos with tracked target overlays (bbox + pose skeleton)"
    )
    parser.add_argument(
        "csv_path",
        type=Path,
    ...
    parser.add_argument(
        "--video-dir",
        type=Path,
        default="/orcd/data/satra/002/datasets/SAILS/Phase_III_Videos/Videos_from_external_standardized",
        help="Base directory containing source videos (used to resolve SourceFile paths from CSV)"
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("rendered_videos"),
        help="Output directory for rendered videos (default: ./rendered_videos)"
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    for idx, row in df.iterrows():
        ...
        # Resolve all paths
        config = resolve_video_config(row, args.dirs, args.video_dir)
        if not config:
            skip_count += 1
            continue
    ...
    logger.info(f"{'='*60}")
    logger.info(f"Total videos in CSV: {len(df)}")
