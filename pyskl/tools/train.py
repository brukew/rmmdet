...
# PyTorch 2.6+ security fix: allow loading checkpoints with numpy arrays
# Monkey-patch torch.load to default to weights_only=False for mmcv compatibility
...
def parse_args():
    ...
    parser.add_argument(
        '--work-dir',
        help='override the work_dir specified in the config file')
    parser.add_argument(
        '--launcher',
        choices=['pytorch', 'slurm', 'none'],
        default='pytorch',
        help='job launcher (use "none" for single-GPU non-distributed training)')
    parser.add_argument(
        '--compile',
        action='store_true',
...
def main():
    ...
    # Merge cfg-options if provided (allows overriding any config value)
    ...
    # CLI --work-dir > config file > default (base filename)
    ...
    # Handle non-distributed (single GPU) training
    ...
    train_model(model, datasets, cfg, validate=args.validate, test=test_option, timestamp=timestamp, meta=meta, distributed=distributed)
