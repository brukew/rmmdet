#!/bin/bash
#SBATCH --job-name=stgcnpp_4cls_cv_w
#SBATCH --partition=mit_preemptable
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=128G
#SBATCH --time=24:00:00
#SBATCH --output=/orcd/data/satra/001/users/brukew/pyskl_logs/stgcnpp/4class_cv_4stream_weighted_%j.out
#SBATCH --error=/orcd/data/satra/001/users/brukew/pyskl_logs/stgcnpp/4class_cv_4stream_weighted_%j.err

echo "=========================================="
echo "STGCN++ Training: 4-class CV (4-Stream) WITH CLASS WEIGHTING"
...
BASE_WORK_DIR="work_dirs/stgcnpp/cv/4class_conf04_weighted"
EPOCHS=24
LR=0.01
...
for FOLD in 0 1 2; do
    ...
    for i in "${!MODALITIES[@]}"; do
        ...
        echo ""
        echo "=========================================="
        echo "Fold ${FOLD}: Training ${MOD_NAME} (${MOD}) WITH CLASS WEIGHTS"
        echo "=========================================="
        
        # Class weights computed dynamically per-fold inside train_weighted.py
...
for mod in modalities:
    ...
    ckpt_pattern = f'work_dirs/stgcnpp/cv/4class_conf04_weighted_{mod}/fold{FOLD}/best_*.pth'
    ckpts = glob.glob(ckpt_pattern)
    ckpt = ckpts[0] if ckpts else f'work_dirs/stgcnpp/cv/4class_conf04_weighted_{mod}/fold{FOLD}/latest.pth'
...
output_dir = Path(f'work_dirs/stgcnpp/cv/4class_conf04_weighted_4stream/fold{FOLD}')
...
plot_confusion_matrix(cm_clip, output_dir / 'confusion_matrix_clip.png', f'Fold {FOLD} - Clip (Weighted)')
plot_confusion_matrix(cm_video, output_dir / 'confusion_matrix_video.png', f'Fold {FOLD} - Video (Weighted)')
...
base_dir = Path('work_dirs/stgcnpp/cv/4class_conf04_weighted_4stream')
folds = []
