#!/bin/bash
#SBATCH --job-name=stgcnpp_4cls_4s
#SBATCH --partition=mit_preemptable
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=128G
#SBATCH --time=12:00:00
#SBATCH --output=/orcd/data/satra/001/users/brukew/pyskl_logs/stgcnpp/4class_single_4stream_%j.out
#SBATCH --error=/orcd/data/satra/001/users/brukew/pyskl_logs/stgcnpp/4class_single_4stream_%j.err

echo "=========================================="
echo "STGCN++ Training: 4-class Single Split (4-Stream)"
...
BASE_WORK_DIR="work_dirs/stgcnpp/single/4class_conf04"
EPOCHS=24
LR=0.01
...
# Video-level aggregation (majority voting)
...
plot_confusion_matrix(cm_clip, output_dir / 'confusion_matrix_clip.png', '4-Stream Fusion - Clip Level')
plot_confusion_matrix(cm_video, output_dir / 'confusion_matrix_video.png', '4-Stream Fusion - Video Level')

# Save predictions CSV (clip-level)
with open(output_dir / 'predictions_clip.csv', 'w', newline='') as f:
    writer = csv.writer(f)
    header = ['segment_id', 'true_label', 'pred_label', 'true_class', 'pred_class'] + [f'score_class{i}' for i in range(len(CLASS_NAMES))]
...
# Save predictions CSV (video-level)
with open(output_dir / 'predictions_video.csv', 'w', newline='') as f:
    writer = csv.writer(f)
    header = ['video_id', 'true_label', 'pred_label', 'true_class', 'pred_class'] + [f'score_class{i}' for i in range(len(CLASS_NAMES))]
    writer.writerow(header)
