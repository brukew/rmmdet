#!/bin/bash
#SBATCH --job-name=stgcnpp_5cls_4s_w
#SBATCH --partition=mit_preemptable
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=128G
#SBATCH --time=12:00:00
#SBATCH --output=/orcd/data/satra/001/users/brukew/pyskl_logs/stgcnpp/5class_single_4stream_weighted_%j.out
#SBATCH --error=/orcd/data/satra/001/users/brukew/pyskl_logs/stgcnpp/5class_single_4stream_weighted_%j.err

echo "=========================================="
echo "STGCN++ Training: 5-class Single Split (4-Stream) WITH CLASS WEIGHTING"
...
BASE_WORK_DIR="work_dirs/stgcnpp/single/5class_conf04_weighted"
EPOCHS=24
LR=0.01
NUM_CLASSES=5
...
for mod in modalities:
    ...
    ckpt_pattern = f'work_dirs/stgcnpp/single/5class_conf04_weighted_{mod}/best_*.pth'
    ckpts = glob.glob(ckpt_pattern)
    ckpt = ckpts[0] if ckpts else f'work_dirs/stgcnpp/single/5class_conf04_weighted_{mod}/latest.pth'
    
    print(f'Loading {mod}: {ckpt}')
    scores, labels, segment_ids = get_predictions_with_ids(config, ckpt, 'data/sails/single/5class_conf04.pkl', NUM_CLASSES)
...
output_dir = Path('work_dirs/stgcnpp/single/5class_conf04_weighted_4stream/eval_test')
...
plot_confusion_matrix(cm_clip, output_dir / 'confusion_matrix_clip.png', '5-Class 4-Stream (Weighted) - Clip')
plot_confusion_matrix(cm_video, output_dir / 'confusion_matrix_video.png', '5-Class 4-Stream (Weighted) - Video')
