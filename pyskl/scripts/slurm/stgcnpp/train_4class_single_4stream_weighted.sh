#!/bin/bash
#SBATCH --job-name=stgcnpp_4cls_4s_w
#SBATCH --partition=mit_preemptable
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=128G
#SBATCH --time=12:00:00
#SBATCH --output=/orcd/data/satra/001/users/brukew/pyskl_logs/stgcnpp/4class_single_4stream_weighted_%j.out
#SBATCH --error=/orcd/data/satra/001/users/brukew/pyskl_logs/stgcnpp/4class_single_4stream_weighted_%j.err

echo "=========================================="
echo "STGCN++ Training: 4-class Single Split (4-Stream) WITH CLASS WEIGHTING"
...
BASE_WORK_DIR="work_dirs/stgcnpp/single/4class_conf04_weighted"
EPOCHS=24
LR=0.01
...
for i in "${!MODALITIES[@]}"; do
    ...
    echo ""
    echo "=========================================="
    echo "Training ${MOD_NAME} (${MOD}) modality WITH CLASS WEIGHTS"
    echo "=========================================="
...
def get_predictions_with_ids(config_path, checkpoint_path, ann_file):
    ...
    test_annotations = [a for a in annotations if a.get('frame_dir', '') in test_ids]
    ...
    with torch.no_grad():
        for data in dataloader:
            ...
            seg_id = test_annotations[idx].get('frame_dir', f'sample_{idx}') if idx < len(test_annotations) else f'sample_{idx}'
...
for mod in modalities:
    ...
    ckpt_pattern = f'work_dirs/stgcnpp/single/4class_conf04_weighted_{mod}/best_*.pth'
    ckpts = glob.glob(ckpt_pattern)
    ckpt = ckpts[0] if ckpts else f'work_dirs/stgcnpp/single/4class_conf04_weighted_{mod}/latest.pth'
    
    print(f'Loading {mod}: {ckpt}')
    scores, labels, segment_ids = get_predictions_with_ids(config, ckpt, 'data/sails/single/4class_conf04.pkl')
...
output_dir = Path('work_dirs/stgcnpp/single/4class_conf04_weighted_4stream/eval_test')
...
plot_confusion_matrix(cm_clip, output_dir / 'confusion_matrix_clip.png', '4-Stream Fusion (Weighted) - Clip Level')
plot_confusion_matrix(cm_video, output_dir / 'confusion_matrix_video.png', '4-Stream Fusion (Weighted) - Video Level')

with open(output_dir / 'predictions_clip.csv', 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['segment_id', 'true_label', 'pred_label', 'true_class', 'pred_class'] + [f'score_class{i}' for i in range(len(CLASS_NAMES))])
    for seg_id, true_label, pred_label, scores in zip(segment_ids, labels, fused_preds, fused_probs):
        writer.writerow([seg_id, true_label, pred_label, CLASS_NAMES[true_label], CLASS_NAMES[pred_label]] + [f'{s:.4f}' for s in scores])

with open(output_dir / 'metrics.json', 'w') as f:
    json.dump({k: float(v) if isinstance(v, (np.floating, float)) else v for k, v in metrics.items()}, f, indent=2)

print(f'\nSaved outputs to: {output_dir}')
print('4-Stream fusion complete!')
"
