#!/bin/bash
#SBATCH --job-name=stgcnpp_4cls_cv_4s
#SBATCH --partition=mit_preemptable
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=128G
#SBATCH --time=12:00:00
#SBATCH --output=/orcd/data/satra/001/users/brukew/pyskl_logs/stgcnpp/4class_cv_4stream_%j.out
#SBATCH --error=/orcd/data/satra/001/users/brukew/pyskl_logs/stgcnpp/4class_cv_4stream_%j.err

echo "=========================================="
echo "STGCN++ Training: 4-class 3-Fold CV (4-Stream)"
...
# Configuration
BASE_WORK_DIR="work_dirs/stgcnpp/cv/4class_conf04"
EPOCHS=24
LR=0.01
...
for FOLD in 0 1 2; do
    ...
    for i in "${!MODALITIES[@]}"; do
        ...
        # Train (no --test-best for CV since there's no test split)
...
for fold in range(3):
    ann_file = f'data/sails/cv/4class_conf04/fold{fold}.pkl'
    fold_probs = []
    ...
    for mod in modalities:
        ...
        ckpt_dir = base_dir.parent / f'4class_conf04_{mod}' / f'fold{fold}'
    ...
    fold_output_dir = base_dir.parent / '4class_conf04_4stream' / f'fold{fold}'
    fold_output_dir.mkdir(parents=True, exist_ok=True)
    
    # Confusion matrices
    cm_clip = confusion_matrix(labels, fused_preds)
    cm_video = confusion_matrix(video_labels, video_preds)
    plot_confusion_matrix(cm_clip, fold_output_dir / 'confusion_matrix_clip.png', f'Fold {fold} - Clip Level')
    plot_confusion_matrix(cm_video, fold_output_dir / 'confusion_matrix_video.png', f'Fold {fold} - Video Level')
...
# Save CV summary
output_dir = base_dir.parent / '4class_conf04_4stream'
