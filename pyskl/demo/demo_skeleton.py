...
def parse_args():
    ...
    parser.add_argument(
        '--use-cache',
        action='store_true',
        help='Use cached detection and pose results')
    parser.add_argument(
        '--cache-base-path',
        type=str,
...
def bottom_pose_inference(args, frame_paths):
    print(f'[DEBUG] Initializing pose model with config: {args.pose_config}')
    print(f'[DEBUG] Loading checkpoint from: {args.pose_checkpoint}')
    print(f'[DEBUG] Using device: {args.device}')
    model = init_pose_model(args.pose_config, args.pose_checkpoint,
                            args.device)
    print('[DEBUG] Pose model initialized successfully')
    ret = []
    print('Performing Human Pose Estimation for each frame')
    prog_bar = mmcv.ProgressBar(len(frame_paths))
    for f in frame_paths:
        # Align input format
        # d = [dict(bbox=x) for x in list(d)]
        pose = inference_bottom_up_pose_model(model, f, return_heatmap=False)[0]
        ret.append(pose)
        prog_bar.update()
    return ret
...
def main():
    ...
    pose_results = bottom_pose_inference(args, frame_paths)
    torch.cuda.empty_cache()
